...
在工作中我需要用 SSH 做远程转发，把本地端口经跳板机转发到内网服务。之前一直用 bash 脚本统一启动这些转发隧道，但始终没有找到一款合适的软件来统一管理和记录这些 SSH 连接。于是我开发了 Via——一个可以统一进行 SSH 远程转发与管理的桌面工具。
...
```
本地客户端 ──► 127.0.0.1:local_port ──► SSH 会话（跳板机）──► target_host:target_port
```
...
| SSH | russh（密码 / 私钥认证） |
| 加密 | Argon2 + XChaCha20-Poly1305 AEAD |
| 本地存储 | SQLite（单个 `via.db` 数据库） |
| 运行时 | Rust 管理配置、加密、SSH、端口监听、状态与重连 |
...
make check      # 快速静态检查
make package    # 生成并 ad-hoc 签名 macOS .app 包
make clean      # 删除 dist 与 Rust target 构建产物
make clean-all  # 在 clean 基础上删除已安装的 JS 依赖
```
...
src-tauri/src/storage/        本地配置、加密与安全导入导出
...
- 导出配置永不携带凭据。
- 密码与私钥口令仅以加密 BLOB 形式写入本地 SQLite。
- 日志不得记录密码、私钥口令或私钥内容。
