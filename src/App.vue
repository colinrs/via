...
const backendTone = computed(() => {
  if (store.initializationState === 'failed') return 'failed'
  if (store.initializationState === 'connecting') return 'connecting'
  return 'ready'
})
...
<style>
:root { color: #e6edf3; background: #0d1117; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; --canvas: #0d1117; --surface: #161b22; --surface-raised: #21262d; --line: #30363d; --text: #e6edf3; --muted: #8b949e; --blue: #388bfd; --green: #3fb950; --red: #f85149; --yellow: #d29922; --radius-sm: 8px; --radius-md: 10px; --radius-lg: 14px; --shadow-soft: 0 6px 18px rgb(0 0 0 / 28%); } * { box-sizing: border-box; } body { margin: 0; min-width: 980px; } button,input,select { font: inherit; } .via-app { display: flex; min-height: 100vh; flex-direction: column; background: var(--canvas); }.app-interactions { display: flex; min-width: 0; min-height: 100vh; flex: 1; flex-direction: column; margin: 0; border: 0; padding: 0; }.titlebar { display: flex; height: 48px; align-items: center; justify-content: space-between; border-bottom: 1px solid var(--line); background: var(--surface); padding: 0 17px; }.brand,.header-actions { display: flex; align-items: center; gap: 10px; }.button-wrap { display: inline-flex; }.brand { font-size: 14px; font-weight: 750; }.mark { display: grid; width: 22px; height: 22px; place-items: center; border-radius: var(--radius-sm); background: var(--blue); color: white; font-size: 12px; }.version { border: 1px solid rgb(56 139 253 / 35%); border-radius: 6px; background: rgb(56 139 253 / 10%); padding: 2px 5px; color: #79c0ff; font-size: 10px; font-weight: 600; }.secondary-button,.success-button,.danger-button { border: 1px solid var(--line); border-radius: var(--radius-md); background: var(--surface-raised); padding: 7px 10px; color: var(--text); font-size: 12px; cursor: pointer; }.secondary-button:hover { border-color: var(--muted); }.primary-button { border: 1px solid #4696fa; border-radius: var(--radius-md); background: #1f6feb; padding: 7px 10px; color: white; font-size: 12px; font-weight: 650; cursor: pointer; }.primary-button:hover { background: #388bfd; box-shadow: var(--shadow-soft); }.success-button { border-color: rgb(63 185 80 / 35%); background: rgb(63 185 80 / 10%); color: #56d364; }.danger-button { border-color: rgb(248 81 73 / 35%); background: rgb(248 81 73 / 8%); color: #ff7b72; }.workspace { display: flex; min-height: 0; flex: 1; }.content { display: flex; min-width: 0; flex: 1; flex-direction: column; }.session-header { display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid var(--line); background: var(--surface); padding: 17px 20px; }.section-label { margin: 0 0 4px; color: var(--blue); font-size: 10px; font-weight: 750; letter-spacing: .08em; text-transform: uppercase; }.session-header h1 { margin: 0; font-size: 16px; }.connection { display: flex; align-items: center; gap: 6px; margin: 5px 0 0; color: var(--muted); font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: 12px; }.statusbar-left { display: flex; align-items: center; gap: 8px; }.backend-dot { display: inline-block; width: 8px; height: 8px; border-radius: 50%; background: var(--green); box-shadow: 0 0 7px var(--green); }.backend-dot.backend-connecting { background: var(--yellow); box-shadow: 0 0 7px var(--yellow); }.backend-dot.backend-failed { background: var(--red); box-shadow: 0 0 7px var(--red); }.connected { color: #56d364; font-size: 12px; }.session-dot { display: inline-block; width: 7px; height: 7px; border-radius: 50%; background: var(--muted); }.session-dot.connected { background: var(--green); box-shadow: 0 0 7px var(--green); }.connection-state { margin-left: 8px; color: var(--muted); }.connection-state.connected { color: var(--green); }button:disabled { opacity: .55; cursor: not-allowed; }.session-editor { border-top: 1px solid var(--line); background: var(--surface); padding: 14px 20px 17px; }.editor-title { margin-bottom: 11px; color: var(--muted); font-size: 12px; font-weight: 700; }.editor-fields { display: grid; grid-template-columns: 1fr 1.3fr 90px 1fr 1.5fr; gap: 12px; }.editor-fields label { display: grid; gap: 5px; color: var(--muted); font-size: 11px; }.editor-fields input,.editor-fields select { min-width: 0; border: 1px solid var(--line); border-radius: var(--radius-sm); outline: 0; background: var(--canvas); padding: 7px 8px; color: var(--text); font-size: 12px; }.editor-fields input:focus,.editor-fields select:focus { border-color: var(--blue); }.editor-fields > button { align-self: end; }.statusbar { display: flex; height: 26px; align-items: center; justify-content: space-between; border-top: 1px solid var(--line); padding: 0 17px; color: var(--muted); font-size: 11px; }.statusbar span { display: flex; align-items: center; gap: 6px; } @media (max-width: 1100px) { .editor-fields { grid-template-columns: repeat(2, 1fr); }.key-path { grid-column: span 2; } }
</style>
