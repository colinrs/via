...
describe('translated component copy', () => {
  it('translates confirmation cancellation and pending action copy', () => {
    ...
    expect(wrapper.get('[data-testid="confirm-dialog-action"]').text()).toBe(
      'Delete in progress…'
    )
  })
  ...
  it('translates group and session creation controls and accessible labels', () => {
    ...
    expect(group.get('[role="dialog"]').attributes('aria-label')).toBe(
      'New group'
    )
    expect(group.get('input').attributes('aria-label')).toBe('Group name')
    expect(group.get('[data-testid="create-group-action"]').text()).toBe(
      'Create group'
    )
    expect(session.get('[role="dialog"]').attributes('aria-label')).toBe(
      'New session'
    )
    expect(session.get('select').attributes('aria-label')).toBe('Group')
    expect(session.get('[data-testid="create-session-action"]').text()).toBe(
      'Create session'
    )
  })
  ...
  it('translates empty-workspace and host-trust copy without changing host details', () => {
    ...
    expect(trust.get('[role="dialog"]').attributes('aria-label')).toBe(
      'Confirm SSH host fingerprint'
    )
  ...
  it('translates import/export explanatory and form copy', () => {
    const exported = mount(ImportDialog, {
      props: { open: true, mode: 'export', exportJson: '{"schemaVersion":1}' },
    })
    const imported = mount(ImportDialog, {
      props: { open: true, mode: 'import' },
    })

    expect(exported.get('[role="dialog"]').attributes('aria-label')).toBe(
      'Export configuration'
    )
    expect(exported.text()).toContain('They never include passwords')
    expect(exported.get('textarea').attributes('aria-label')).toBe(
      'Configuration JSON'
    )
    expect(imported.get('textarea').attributes('placeholder')).toBe(
  ...
  it('translates recovery, setup, unlock, and recovery-mode credential forms', async () => {
    const codes = mount(RecoveryCodesDialog, {
      props: { open: true, codes: ['CODE-1'] },
    })
    const setup = mount(SecretSetupDialog, { props: { open: true } })
    const unlock = mount(SecretUnlockDialog, { props: { open: true } })

    expect(codes.get('[role="dialog"]').attributes('aria-label')).toBe(
      'Save recovery codes'
    )
    expect(codes.get('input').attributes('aria-label')).toBe(
      'I saved the recovery codes'
    )
    expect(setup.get('[role="dialog"]').attributes('aria-label')).toBe(
      'Set up local credentials'
    )
    expect(setup.get('input').attributes('aria-label')).toBe('Master password')
    expect(unlock.get('[role="dialog"]').attributes('aria-label')).toBe(
      'Unlock local credentials'
    )
    expect(unlock.get('input').attributes('aria-label')).toBe(
      'App master password'
    )
  ...
  it('translates sidebar controls and status accessibility copy', () => {
        ...
        groups: [
          {
            ...
            name: 'Production',
            icon: '▣',
            sessions: [{ id: 'session', name: 'Bastion', state: 'stopped' }],
          },
        ],
      },
    })
    ...
    expect(
      wrapper.get('[data-testid="delete-group-group"]').attributes('aria-label')
    ).toBe('Delete group Production')
    expect(
      wrapper.get('[data-testid="delete-group-group"]').attributes('title')
    ).toBe('Delete group')
    expect(wrapper.get('.session-indicator').attributes('aria-label')).toBe(
      'Stopped'
    )
  })
  ...
  it('translates tunnel table, fields, state labels, actions, and diagnostics', () => {
    ...
    expect(wrapper.get('input[placeholder]').attributes('placeholder')).toBe(
      'Search ports, target hosts, or notes'
    )
    ...
    expect(wrapper.get('.diagnostic').text()).toContain(
