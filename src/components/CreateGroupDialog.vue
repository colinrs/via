...
<template>
  <div
    ...
    role="dialog"
    aria-modal="true"
    :aria-label="t('dialog.newGroup.title')"
  >
    <section class="dialog">
      <h2>{{ t('dialog.newGroup.title') }}</h2>
      <p>{{ t('dialog.newGroup.description') }}</p>
      <label
        >{{ t('field.groupName')
        }}<input
          v-model="name"
          ...
          autofocus
          @keyup.enter="normalizedName && emit('create', normalizedName)"
      /></label>
      <footer>
        ...
        ><button
          ...
          @click="emit('create', normalizedName)"
        >
