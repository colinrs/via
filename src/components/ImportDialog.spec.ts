...
describe('ImportDialog', () => {
  it('never offers exporting credentials', () => {
    const wrapper = mount(ImportDialog, {
      ...withChineseI18n(),
      props: { mode: 'export', open: true },
    })
    expect(wrapper.text()).toContain('不会包含密码')
    expect(wrapper.text()).not.toMatch(/携带凭据|导出凭据/)
  })
  it('fills the textarea with a demo config in import mode', async () => {
    const wrapper = mount(ImportDialog, {
      ...withChineseI18n(),
      props: { mode: 'import', open: true },
    })
