...
describe('SecretUnlockDialog', () => {
  it('emits a nonblank password from unlock mode and clears it', async () => {
    const wrapper = mount(SecretUnlockDialog, {
      ...withChineseI18n(),
      props: { open: true },
    })
  ...
  it('emits recovery input only from recovery mode', async () => {
    ...
    expect(
      (wrapper.get('[aria-label="恢复码"]').element as HTMLInputElement).value
    ).toBe('')
    expect(
      (wrapper.get('[aria-label="新主密码"]').element as HTMLInputElement).value
    ).toBe('')
  })
  ...
  it('clears every secret when switching modes, closing, or reopening', async () => {
    ...
    expect(
      (wrapper.get('[aria-label="应用主密码"]').element as HTMLInputElement)
        .value
    ).toBe('')
  ...
  it('uses credential-specific autocomplete hints in recovery mode', async () => {
    ...
    await wrapper.get('[data-testid="show-recovery"]').trigger('click')
    expect(
      wrapper.get('[aria-label="恢复码"]').attributes('autocomplete')
    ).toBe('one-time-code')
    expect(
      wrapper.get('[aria-label="新主密码"]').attributes('autocomplete')
    ).toBe('new-password')
    expect(
      wrapper.get('[aria-label="确认新主密码"]').attributes('autocomplete')
    ).toBe('new-password')
  })
})
