...
const groups = [
  {
    ...
    icon: '▣',
    sessions: [
      { id: 'session-a', name: 'SSH 会话', state: 'stopped' as const },
    ],
  },
]
...
describe('SessionSidebar', () => {
  it('renders groups expanded as tree parents initially', () => {
    ...
    expect(
      wrapper
        .get('[data-testid="group-toggle-group-a"]')
        .attributes('aria-expanded')
    ).toBe('true')
    expect(
      wrapper.get('[data-testid="session-child-session-a"]').isVisible()
    ).toBe(true)
  })
  ...
  it('hides and restores only the toggled group children', async () => {
    ...
    expect(
      wrapper.find('[data-testid="session-child-session-a"]').exists()
    ).toBe(false)
    await toggle.trigger('click')
    expect(
      wrapper.get('[data-testid="session-child-session-a"]').isVisible()
  ...
  it('reopens the parent when its session becomes selected', async () => {
    ...
    expect(
      wrapper.get('[data-testid="session-child-session-a"]').classes()
    ).toContain('selected')
  })
