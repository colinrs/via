...
watch(
  ...
  (sessionId) => {
    const parent = props.groups.find((group) =>
...
<template>
  <aside data-testid="session-sidebar" class="sidebar">
    <button
      class="primary-button create-session"
      type="button"
      @click="emit('create')"
    >
      <span aria-hidden="true">＋</span> {{ t('action.newSshSession') }}
    </button>
    <button class="create-group" type="button" @click="emit('createGroup')">
      <span aria-hidden="true">＋</span> {{ t('action.newGroup') }}
    </button>
        ...
        <div v-if="isExpanded(group.id)" class="tree-children">
          ...
          >
            ...
            <span
              ...
              :class="session.state"
              :aria-label="t(`state.${session.state}`)"
            />
          </button>
        </div>
      </section>
...
.group-heading[aria-expanded='true']::before {
